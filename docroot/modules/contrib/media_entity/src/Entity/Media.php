...
class Media extends ContentEntityBase implements MediaInterface {
  ...
  /**
   * Get original media entity from storage.
   *
   * @return \Drupal\Core\Entity\EntityInterface|null
   *   Returns original media entity object.
   */
  protected function getOriginal() {
    $id = $this->getOriginalId() !== NULL ? $this->getOriginalId() : $this->id();

    return $this->entityManager()->getStorage($this->getEntityTypeId())->loadUnchanged($id);
  }
  ...
  public function preSave(EntityStorageInterface $storage) {
    ...
    // Set thumbnail.
    if (!$this->get('thumbnail')->entity || !empty($this->queued_thumbnail_download) || $this->getType()->shouldUpdateThumbnail($this, $this->getOriginal())) {
